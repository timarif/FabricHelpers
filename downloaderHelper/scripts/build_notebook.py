...
def cells(version: str) -> list[dict[str, str]]:
    return [
        ...
        _md(
            ...
            "other Spark languages). Use `\"ipynb\"` for a single",
            "self-contained Jupyter file or `\"parts\"` to write every",
            "definition part (same shape the other item types get).",
            "Non-notebook types use `format_by_type` for per-type",
            "`?format=` overrides; missing keys fall back to parts mode.",
        ),
        _code(
            ...
            "    notebook_format = \"py\",   # \"py\" | \"ipynb\" | \"parts\"",
